...
\paragraph{Theoretical Compression of RLE Components}
...
\paragraph{Theoretical Random Access in RLE}
...
\todo[inline]{Adesso qui dovrei aggiungere diciamo l'unica parte sperimentale dove inserisco per certi dataset i risultati dei mockup di memory usage per quanto riguarda il dag succinto con RLE e senza RLE. Si giustifica che i nostri valori sono effettivamente sotto il valore di entropia (che devo pure calcolare per quel dag) semplicemente per il fatto che quello è il minimo teorico per una compressione lossless del DAG. Noi siamo lossy, non possiamo in particolare ricostruire il DAG originale, ma possiamo comunque calcolare il rank.}
