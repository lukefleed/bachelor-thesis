...
\subsection{Select}
% The Select operation is the inverse of the Rank operation: given a bit $b$ and an index $i$, it returns the position of the $i$-th occurrence of $b$ in $B$. We can notice that $rank$ is a nondecreasing function, so we can use binary search to find the position of the $i$-th occurrence of $b$ in $B$. The Select operation can be implemented in $O(\log n)$ time, but we can achieve constant time by using the same hierarchical structure as the Rank operation. \vspace{0.4cm}
...
\todo[inline,color=blue!30]{Do I talk about select in detail? Or I can just say that mainly follows the three-level design of the Rank data structure, with the algorithmic twist that here the binary array B is not split into big and small blocks of fixed length, but the splitting is driven by the number of bits set to 1?}
